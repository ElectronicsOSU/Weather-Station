...
void setup() {
  ...
  attachInterrupt(pushbutton,changeread,CHANGE);
...
void changeread()
{
  ...
  /* these change the LEDs to determine which sensor is being display i.e. if green is on, temp in F is dislplayed
  if(flag == 0)
  {
    flag = 1;
    digitalWrite(CTempInd,HIGH);
    digitalWrite(FTempInd,LOW);
    current_mode = CTempInd;
  }
  else if(flag==1)
  {
    flag = 2;
    digitalWrite(CTempInd,LOW);
    digitalWrite(FTempInd,HIGH);
    current_mode = FTempInd;
  }
    else if(flag==2)
  {
    flag = 0;
